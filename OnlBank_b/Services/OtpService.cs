...
namespace OnlBank_b.Services
{
    public class OtpService
    {
        private readonly BankingonlineContext _context;

        public OtpService(BankingonlineContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> CheckOtp(string otp, int userId)
        {
            ...
            var timeDifference = currentTime - code.CreateTime;
        ...
        {
            string filePath = Path.Combine(Directory.GetCurrentDirectory(), "Services/Form-Email/otp_email_template.html");
